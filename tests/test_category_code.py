...
class CategoryCodeApiTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_get_by_id_embeds_in_code_set_object_list_stays_flat(self):
        ...
        self.assertEqual(embedded["@type"], "CategoryCodeSet")
        ...
        self.assertIsInstance(in_list["inCodeSet"], str)

    def test_get_by_id_leaves_dangling_in_code_set_ref_as_uuid(self):
        dangling = "00000000-0000-0000-0000-000000000000"
        payload = build_payload(self.server, ENTITY, partial=True)
        payload["inCodeSet"] = dangling
        created = request_json(self.server, "POST", BASE, payload)["body"]
        got = request_json(self.server, "GET", f"{BASE}/{created['id']}")["body"]
        self.assertEqual(got["inCodeSet"], dangling)
