...
class BlogPostingApiTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_sort_by_word_count_numeric_with_missing_last(self):
        ids = []
        for v in [100, 0, 9, 99]:
            payload = build_payload(self.server, ENTITY)
            payload["wordCount"] = v
        ...
        res = request_json(self.server, "GET", f"{BASE}?sort=wordCount&order=asc&limit=100")
        self.assertEqual(res["status"], 200)
        id_set = set(ids)
        mine = [i for i in res["body"]["items"] if i.get("id") in id_set]
        present_vals = [
            i["wordCount"] for i in mine
            if isinstance(i.get("wordCount"), (int, float)) and not isinstance(i.get("wordCount"), bool)
        ]
    ...
    def test_get_by_id_embeds_image_as_list_of_objects(self):
        payload = build_payload(self.server, ENTITY, partial=True)
        created = request_json(self.server, "POST", BASE, payload)["body"]
        got = request_json(self.server, "GET", f"{BASE}/{created['id']}")["body"]
        embedded = got["image"]
