...
class ImageObjectApiTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_get_by_id_embeds_creator_object_list_stays_flat(self):
        ...
        self.assertIsInstance(in_list["creator"], str)

    def test_get_by_id_leaves_dangling_creator_ref_as_uuid(self):
        dangling = "00000000-0000-0000-0000-000000000000"
        payload = build_payload(self.server, ENTITY, partial=True)
        payload["creator"] = dangling
        created = request_json(self.server, "POST", BASE, payload)["body"]
        got = request_json(self.server, "GET", f"{BASE}/{created['id']}")["body"]
        self.assertEqual(got["creator"], dangling)
