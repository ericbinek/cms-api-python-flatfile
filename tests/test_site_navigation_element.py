...
class SiteNavigationElementApiTest(unittest.TestCase):
    @classmethod
    def setUpClass(cls):
    ...
    def test_sort_by_position_numeric_with_missing_last(self):
        ids = []
        for v in [100, 0, 9, 99]:
            payload = build_payload(self.server, ENTITY)
            payload["position"] = v
        ...
        res = request_json(self.server, "GET", f"{BASE}?sort=position&order=asc&limit=100")
        self.assertEqual(res["status"], 200)
        id_set = set(ids)
        mine = [i for i in res["body"]["items"] if i.get("id") in id_set]
        present_vals = [
            i["position"] for i in mine
            if isinstance(i.get("position"), (int, float)) and not isinstance(i.get("position"), bool)
        ]
    ...
    def test_get_by_id_embeds_is_part_of_object_list_stays_flat(self):
        ...
        self.assertEqual(embedded["@type"], "SiteNavigationElement")
        ...
        self.assertIsInstance(in_list["isPartOf"], str)

    def test_get_by_id_leaves_dangling_is_part_of_ref_as_uuid(self):
        dangling = "00000000-0000-0000-0000-000000000000"
        payload = build_payload(self.server, ENTITY, partial=True)
        payload["isPartOf"] = dangling
        created = request_json(self.server, "POST", BASE, payload)["body"]
        got = request_json(self.server, "GET", f"{BASE}/{created['id']}")["body"]
        self.assertEqual(got["isPartOf"], dangling)
